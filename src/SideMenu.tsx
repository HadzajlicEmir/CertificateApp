...
<div onClick = {()=> menuClick(item.isDropDown)} key = {item.Label}>
    ...
    {item.Icon} {item.isDropDown ? <div>{item.Label} <ExpandMoreIcon /></div> : <Link style= {{textDecoration: 'none', color: '#3f9ac9'}} to={"/".concat(item.Label.toLowerCase())}>{item.Label}</Link>}
    </div>
    {submenuOpen ? <div onClick={(event) => subMenuItemCLick(event)} style={{ marginLeft: "45px", color: '#3f9ac9'}}>
        {item.submenuItems?.map(subItem => <div><Link style= {{textDecoration: 'none', color: '#3f9ac9'}} to={"/".concat(subItem.replaceAll(" ", "").toLowerCase())}>{t(subItem)}</Link></div>)}
        </div> : undefined}
</div>))}
</div>
)
